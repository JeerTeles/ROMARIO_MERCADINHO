...
document.addEventListener('DOMContentLoaded', () => {
    const API_CLIENTES_URL = 'http://localhost:3000/api/clientes';
    const API_VERIFY_PASSWORD_URL = 'http://localhost:3000/api/admin/verify-password';
    const API_ESTOQUE_LIST_URL = 'http://localhost:3000/api/estoque/list-for-select'; // Rota para pegar nome e ID do estoque
    const CLIENTS_PER_PAGE = 10;
    ...
    // --- Carregar e popular o dropdown de itens do estoque ---
    async function populateItemSelect() {
        try {
            const response = await fetch(API_ESTOQUE_LIST_URL);
            if (!response.ok) {
            ...
            // Limpa as opções existentes (exceto a primeira "Selecione...")
            itemClienteSelect.innerHTML = '<option value="">Selecione um Item (Opcional)</option>'; // Value vazio

            items.forEach(item => {
                const option = document.createElement('option');
                option.value = item.produto; // O VALOR DA OPÇÃO AGORA É O NOME DO PRODUTO
                option.textContent = item.produto; // O texto visível continua sendo o nome do produto
                itemClienteSelect.appendChild(option);
            });

        } catch (error) {
            console.error('Erro ao carregar itens do estoque para seleção:', error);
            itemClienteSelect.innerHTML = '<option value="" disabled>Erro ao carregar itens</option>';
        }
    }
    ...
    function renderClientsList(clients) {
        ...
        clients.forEach((client) => {
            ...
            // Exibe o nome do produto diretamente de client.item (já é o nome)
            const itemCell = document.createElement('td');
            itemCell.textContent = client.item || 'N/A'; // client.item já é o nome
            row.appendChild(itemCell);
    ...
    addRecordBtn.addEventListener('click', async () => {
        ...
        const telefoneInputVal = telefoneClienteInput.value.trim();
        const cpfCliente = cpfClienteInput.value.trim();
        const itemCliente = itemClienteSelect.value;       // Pega o NOME do produto (value)
        const quantidadeCliente = quantidadeClienteInput.value.trim();
        const dividaCliente = dividaClienteInput.value.trim();
        ...
        // Se o valor do select é vazio e o usuário não quer um item, tudo bem.
        const itemToSave = itemCliente === "" ? "" : itemCliente; // Envia string vazia se "Selecione..."

        const clientData = {
            nomeCliente,
            telefone,
            cpf: cpfCliente,
            item: itemToSave,        // NOVO: Envia o NOME do produto
            quantidade: parseInt(quantidadeCliente),
            divida: parseFloat(dividaCliente)
        };
    ...
    async function editRecord(client) {
        ...
        try {
            ...
            telefoneClienteInput.value = client.telefone;
            cpfClienteInput.value = client.cpf;
            // Define o valor do select para o NOME do item
            itemClienteSelect.value = client.item || ''; // client.item já é o nome
            quantidadeClienteInput.value = client.quantidade;
            dividaClienteInput.value = client.divida;
    ...
    function clearForm() {
        ...
        telefoneClienteInput.value = '';
        cpfClienteInput.value = '';
        itemClienteSelect.value = '';       // NOVO: Reseta o select para vazio
        quantidadeClienteInput.value = '';
        dividaClienteInput.value = '';
        telefoneClienteInput.style.borderColor = '';
        telefoneClienteInput.title = '';
