...
const db = new sqlite3.Database(DB_FILE, (err) => {
    if (err) {
        ...
        db.run(`CREATE TABLE IF NOT EXISTS clientes (
            ...
            divida REAL DEFAULT 0.0, -- Dívida geral, será calculada
            itens_associados TEXT DEFAULT '[]' -- Armazenará um JSON array
        )`, (err) => {
...
// MODIFICADA: Adicionar um novo cliente (dívida inicial 0, calculada posteriormente)
app.post('/api/clientes', (req, res) => {
    let { nomeCliente, telefone, cpf } = req.body; // 'divida' não vem mais do frontend diretamente

    telefone = String(telefone).replace(/\D/g, '');
    const divida = 0.0; // Dívida inicial 0.0
    ...
    db.run(`INSERT INTO clientes (nomeCliente, telefone, cpf, divida, itens_associados) VALUES (?, ?, ?, ?, ?)`,
        ...
        function (err) {
            ...
            res.status(201).json({ id: this.lastID, ...req.body, divida: divida, itens_associados: itens_associados }); // Retorna dados completos
        }
    );
});

// MODIFICADA: Atualizar um cliente existente (dívida é atualizada automaticamente)
app.put('/api/clientes/:id', (req, res) => {
    const { id } = req.params;
    // 'divida' não vem mais do frontend diretamente. 'itens_associados' pode vir se o frontend tiver atualizado.
    let { nomeCliente, telefone, cpf, itens_associados } = req.body; 
    ...
    db.get('SELECT itens_associados FROM clientes WHERE id = ?', [id], (err, currentClient) => {
        ...
        // Se itens_associados veio no body e é válido, usa. Senão, usa o do DB.
        ...
        // Calcula a dívida com base na lista de itens (seja a nova ou a atual do DB)
        ...
        db.run(`UPDATE clientes SET nomeCliente = ?, telefone = ?, cpf = ?, divida = ?, itens_associados = ? WHERE id = ?`,
            ...
            function (err) {
                ...
                if (this.changes === 0) { res.status(404).json({ error: 'Cliente não encontrado.' }); } else { res.json({ message: 'Cliente atualizado com sucesso.', divida: novaDivida }); } // Retorna a nova dívida
            }
        );
    });
});
...
// MODIFICADA: POST: Adicionar um item à lista de itens_associados de um cliente
// Agora, após adicionar, recalcula e atualiza a dívida do cliente
app.post('/api/clientes/:id/itens', (req, res) => {
    ...
    db.get('SELECT itens_associados FROM clientes WHERE id = ?', [clientId], (err, clientRow) => {
        ...
        db.get('SELECT produto, precoDeVenda FROM estoque WHERE id = ?', [produto_id], (err, productRow) => {
            ...
            const novaDivida = calculateTotalDivida(updatedItensAssociados); // Recalcula a dívida
...
// MODIFICADA: DELETE: Remover um item da lista itens_associados de um cliente
// Agora, após remover, recalcula e atualiza a dívida do cliente
